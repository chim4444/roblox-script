...
-- ===================== AUTO ESCAPE =====================
local function getEscapeRemote()
	local char = LP.Character
	if not char then return nil end
	local main = char:FindFirstChild("MainModule")
	if not main then return nil end
	local abilities = main:FindFirstChild("Abilities")
	if not abilities then return nil end
	local ability3 = abilities:FindFirstChild("Ability3")
	if not ability3 then return nil end
	return ability3:FindFirstChild("UseAbility")
end
...
local function updateItemESP()
	...
	for _, obj in ipairs(Workspace:GetDescendants()) do
		...
		-- Gas Canister / Medkit / Wirebox
		if obj:IsA("Model") or obj:IsA("Folder") then
			local color = nil

			if name:find("gascanister") or name:find("gas canister") then
				color = Color3.fromRGB(255, 170, 0) -- Orange
			elseif name == "medkit" then
				color = Color3.fromRGB(0, 255, 100) -- Green
			elseif name == "wirebox" then
...
local function getEntityPlayer()
	local highestHP = 0
	local entityPlayer = nil
	for _, player in ipairs(Players:GetPlayers()) do
		if player.Character then
			local hum = player.Character:FindFirstChildOfClass("Humanoid")
			if hum and hum.MaxHealth > highestHP then
...
local function updateESP()
	...
	local entityPlayer = getEntityPlayer()

	if entityPlayer ~= lastEntity then
		for plr, _ in pairs(highlights) do
			cleanup(plr)
		end
		lastEntity = entityPlayer
	end

	for _, player in ipairs(Players:GetPlayers()) do
		if player ~= LP then
			if player.Character and player.Character:FindFirstChild("Humanoid") and player.Character:FindFirstChild("HumanoidRootPart") then
				local isEnt = (player == entityPlayer)
...
local function switchTab(name)
	...
	elseif name == "Survivor" then
		...
		makeToggle("Auto Escape", autoEscape, function(v) autoEscape = v end)
		makeToggle("Auto Objective", autoObjective, function(v) autoObjective = v end)
	...
	elseif name == "Visuals" then
		...
		makeToggle("Item ESP", itemESPEnabled, function(v)
			itemESPEnabled = v
...
local tabs = {"Entity", "Survivor", "Visuals"}
...
switchTab("Survivor")
print("TIMELESS Hub loaded - Wirebox ESP added")
